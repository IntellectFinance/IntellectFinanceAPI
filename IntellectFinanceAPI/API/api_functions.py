...
def fundamental_metrics(*ignore, cik_or_ticker, metric_name, historical_or_latest=None): 
    ...
    :param historical_or_latest: Optional (default value is `LATEST`). Choose to retrieve either the value for the latest quarter (`LATEST`), or the historical values (`HISTORICAL`) for the last 4.0 years. Note that some metrics, such as PE, require the stock price information. If you choose `LATEST`, we will use the latest stock price (which may change during the day). If you choose `HISTORICAL`, we will use the latest end-of-day stock price for the latest quarter; For other quarters, we use the volume-weighted average end-of-day stock price one month after the fiscal quarter ends. For example, if the fiscal quarter ends at 12/31/2022, we will use the stock price from 02/01/2023 - 02/28/2023.
